...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const codeRes = await fetch('/api/generate-code', { method: 'POST', body: JSON.stringify({ figmaJson: figmaData }), headers: { 'Content-Type': 'application/json' } });
      const { code } = await codeRes.json();

      if (!codeRes.ok) throw new Error(code.error);

      setStatus('Saving component...');
      const saveRes = await fetch('/api/save-component', { method: 'POST', body: JSON.stringify({ projectName, componentName: 'FigmaComponent', code }), headers: { 'Content-Type': 'application/json' } });
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6">
          ...
          {status && <p className={`mt-4 p-2 rounded ${status.includes('Error') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{status}</p>}
        </div>
      </div>
    </main>
